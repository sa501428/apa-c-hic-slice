// Filters a HiC Slice file for contacts whose first or second bin falls within regions specified in a BED file.
// Usage: bed_slice_filter <input.slice> <regions.bed> <output.slice> [--gz]
//
// Compile with: g++ -std=c++11 -lz -o bed_slice_filter bed_slice_filter.cpp
...
int main(int argc, char** argv) {
    ...
    while (true) {
        ...
        if (it1 != bedBins.end() && it1->second.count(rec.binX)) {
            keep = true;
        } else {
            auto it2 = bedBins.find(rec.chr2Key);
            if (it2 != bedBins.end() && it2->second.count(rec.binY)) {
                keep = true;
            }
        }
        if (!keep) continue;
